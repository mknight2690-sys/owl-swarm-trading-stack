...
funding_favorable_short = fr >= -0.0005
...
if funding_favorable_short:
    notes_parts.append(f"funding {fr:.6f} favorable for short (longs pay)")
else:
    notes_parts.append(
        f"ERROR: funding {fr:.6f} below -0.0005; unfavorable for short"
    )

if not trend_match_short:
    notes_parts.append(
        f"ERROR: 1m={t1['bias']} 15m={t15['bias']} 1h={t1h['bias']}; short needs all bearish"
    )
else:
