...
suggested = "short" if conf >= 0.5 and t15["bias"] == "short" and t1h["bias"] == "short" else (
    "long" if conf >= 0.5 and all(x["bias"] == "long" for x in [t1, t15, t1h]) else "neutral"
)
...
tech = (
    ...
    f"24h -{abs(change24h):.2f}% but 1m/15m bounce vs 1h bearish; no MTF alignment. "
    f"Funding {fr:.6f} positive (shorts pay). Spread OK. FIGHT on avoid list."
)
