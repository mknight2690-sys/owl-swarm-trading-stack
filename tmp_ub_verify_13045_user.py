...
if conf < 0.5:
    analysis_issues.append(f"confidence {conf} below 0.5 threshold")
if ta1["rsi"] and ta1["rsi"] > 70:
    analysis_issues.append(f"1m RSI {ta1['rsi']} overbought")
if ta1h["rsi"] and ta1h["rsi"] > 75:
    analysis_issues.append(f"1h RSI {ta1h['rsi']} overbought")
if change24h > 40:
    analysis_issues.append(f"extended 24h move {change24h:.1f}% increases reversal risk")
if abs(price - ENTRY) / ENTRY * 100 > 1:
    analysis_issues.append(f"live price {price:.5f} differs from quoted {ENTRY}")
verified = len(issues) == 0
if not favorable_long or not spread_ok:
    suggested = "neutral"
elif conf >= 0.5 and ta1["bias"] == "long" and ta15["bias"] == "long" and ta1h["bias"] == "long":
    suggested = "long"
elif conf >= 0.5:
    suggested = "neutral"
else:
...
if analysis_issues:
    note_bits.extend(analysis_issues)
if issues:
    note_bits.extend(issues)
elif not analysis_issues:
