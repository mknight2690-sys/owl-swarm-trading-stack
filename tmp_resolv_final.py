...
m1, r1, t1, cl1 = mom(c1m)
m15, r15, t15, cl15 = mom(c15)
m1h, r1h, t1h, cl1h = mom(c1h)
...
vol24h = float(ticker.get("volCurrency24h", ticker.get("vol24h", 0)))
...
if avoid_spread:
    ...
    if t1 == "bearish" and t15 == "bearish":
        ...
        confidence -= 0.08  # MTF conflict
    if r15 and r15 > 65:
        confidence += 0.05  # overbought pullback
    if fund_rate < 0:
        confidence -= 0.05  # funding against short
elif overall == "mixed":
    if m1 >= 0.58 and m15 >= 0.52 and not avoid_long and t1h != "bearish":
        suggested, confidence = "long", 0.54
    elif m1 <= 0.42 and m15 <= 0.48 and not avoid_short and t1h != "bullish":
        suggested, confidence = "short", 0.54
    else:
        suggested = "long" if avg_mom >= 0.5 else "short"
        confidence = 0.38

# scanner long_score 0.91 vs short 0.42
if suggested == "long":
    confidence += 0.04
else:
    confidence -= 0.03
# positive RESOLV track record
confidence += 0.03
# recent long loss caution
if suggested == "long":
    confidence -= 0.04
...
print(json.dumps({
    ...
    "debug": {"t1": t1, "t15": t15, "t1h": t1h, "m1": m1, "m15": m15, "m1h": m1h}
}, separators=(",", ":")))
