...
change24h = round(float(ticker.get("chg_pct", 12.112)), 3)
volume24h = float(ticker.get("vol24h", 28815048))

summary = (
    f"1m {t1} RSI {fmt_r(r1)}, 15m {t15} RSI {fmt_r(r15)}, 1h {t1h} RSI {fmt_r(r1h)}; "
    f"24h +{change24h:.1f}%; spread {spread:.3f}%; funding benign; MTF {overall}"
)
