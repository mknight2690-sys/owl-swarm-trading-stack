...
trends = [ta1["bias"], ta15["bias"], ta1h["bias"]]
if all(t == "long" for t in trends):
    trend = "bullish"
elif all(t == "short" for t in trends):
    trend = "bearish"
elif trends.count("long") >= 2:
    trend = "bullish"
elif trends.count("short") >= 2:
    trend = "bearish"
else:
    trend = "mixed"

mom_score = round((ta1["tech"] + ta15["tech"] + ta1h["tech"]) / 3, 3)
key_levels = sorted(
    {
        round(ta1h["low"], 5),
        round(ta15["low"], 5),
...
tech_sum = (
    ...
    f"spread {spread:.4f}%; funding {fr:.6f}; 24h {change24h:+.2f}%"
)
...
if conf >= 0.5 and spread_ok:
    if aligned_long and favorable_long:
        ...
        suggested = "short"
    elif ta15["bias"] == "long" and ta1h["bias"] == "long" and favorable_long:
        suggested = "long"
    elif ta15["bias"] == "short" and ta1h["bias"] == "short" and favorable_short:
        suggested = "short"
    elif ta1["tech"] >= 0.55 and favorable_long:
        suggested = "long"
    elif ta1["tech"] <= 0.45 and favorable_short:
        suggested = "short"
    else:
        suggested = "long" if change24h > 0 and favorable_long else "short"

if not spread_ok or conf < 0.5:
    suggested = "long" if conf >= 0.5 else suggested
if suggested == "long" and (not favorable_long or not spread_ok):
    suggested = "short" if favorable_short and spread_ok else suggested
if suggested == "short" and (not favorable_short or not spread_ok):
    suggested = "long" if favorable_long and spread_ok else suggested
if conf < 0.5:
    suggested = "long"
