...
if not trend_match_short:
    notes.append("ERROR: not all timeframes bearish/short aligned")
if t15["mom5"] > 0 or t1h["mom5"] > 0:
    notes.append("ERROR: 15m/1h positive mom5 conflicts with short momentum claim")
notes.append(f"liq distance {liq_dist:.2f}% at 10x isolated short entry {ENTRY}")
if liq_dist < 5:
    notes.append("HIGH LIQUIDATION RISK")
notes.append(f"live price {float(ticker['last'])} 24h {change24h:.2f}%")
notes.append(
    "HALLUCINATION/ASSUMPTION: FIGHT-USDT poor track record 1 trade W/L=0/1 pnl=-0.0105; avoid repeat loser"
)
