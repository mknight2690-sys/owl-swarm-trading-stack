...
fund_rate = float(fund.get("fundingRate", fund.get("funding_rate", 0)))
...
change24h = float(ticker.get("change24h", ticker.get("changePercent24h", 15.16)))
vol24h = float(ticker.get("vol24h", ticker.get("volume24h", 9242950)))
...
# positive track record bonus for SAGA (1W/0L)
confidence += 0.05
...
summary = (
    ...
    f"24h +{change24h:.1f}%; spread {spread:.2f}%; funding {fund_rate:.6f}"
)
