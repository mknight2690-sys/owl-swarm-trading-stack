...
fund = fetch("get_funding_rate", {"inst_id": "W-USDT"})
book = fetch("get_order_book", {"inst_id": "W-USDT"})
ticker = fetch("get_ticker", {"inst_id": "W-USDT"})
...
c1m = parse_candles(fetch("get_candles", {"inst_id": "W-USDT", "bar": "1m", "limit": "50"}))
c15 = parse_candles(fetch("get_candles", {"inst_id": "W-USDT", "bar": "15m", "limit": "30"}))
c1h = parse_candles(fetch("get_candles", {"inst_id": "W-USDT", "bar": "1H", "limit": "24"}))
...
align_note = (
    ...
    if aligned_bull or aligned_bear
    else "Timeframes mixed — partial pullback within 24h uptrend."
)
summary = (
    ...
    json.dumps(
        {
            ...
            "suggestedSide": side if conf >= 0.5 else "neutral",
            "confidence": conf,
        }
    )
)
