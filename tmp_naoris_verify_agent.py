...
if CLAIMED_SIDE == "long" and t1["bias"] != "long":
    issues.append(f"1m trend {t1['bias']} conflicts with long")
if CLAIMED_SIDE == "long" and t15["bias"] != "long":
    issues.append(f"15m momentum {t15['bias']} conflicts with long")
if CLAIMED_SIDE == "long" and t1h["bias"] != "long":
    issues.append(f"1h trend {t1h['bias']} conflicts with long")
if liq_dist < 5:
    issues.append("HIGH LIQUIDATION RISK")
if abs(price - 0.04118) / 0.04118 > 0.01:
    issues.append(f"claimed price 0.04118 vs live {price:.5f}")
if abs(change24h - 34.972) > 1.0:
    issues.append(f"claimed 24h change 34.972% vs live {change24h:.2f}%")
if abs(vol - 4620010) / 4620010 > 0.05:
    issues.append(f"claimed volume 4620010 vs live {vol:.0f}")
