...
change24h = round(float(ticker.get("chg_pct", 13.23)), 3)
volume24h = float(ticker.get("vol24h", 28845656))

summary = (
    f"1m {t1} RSI {int(r1) if r1 else 'NA'}, 15m {t15} RSI {int(r15) if r15 else 'NA'}, "
    f"1h {t1h} RSI {int(r1h) if r1h else 'NA'}; 24h +{change24h:.1f}%; "
    f"spread {spread:.3f}%; funding benign; MTF {overall}"
)
