...
funding_label = "favorable" if favorable_long else "unfavorable"
note_bits = [
    ...
    f"funding {funding_label} for long (rate {fr:.6f})",
]
core_issues = [i for i in issues if "order_blocked" not in i]
if core_issues:
    note_bits.extend(core_issues)
else:
    note_bits.append("All checks pass")
note_bits.append("ID-USDT prior order_blocked; no positive track record (0 trades, blocked=1)")
