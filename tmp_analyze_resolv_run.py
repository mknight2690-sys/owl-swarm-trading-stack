...
c1h_closes = [x["c"] for x in d1h]
ema9_1h = ema(c1h_closes, 9) if len(c1h_closes) >= 9 else price
ema21_1h = ema(c1h_closes, 21) if len(c1h_closes) >= 21 else price
trend_1h_bull = ema9_1h > ema21_1h

factors = [
    0.7 if ema9 > ema21 else 0.35,
    0.55 if 45 <= rsi <= 60 else (0.65 if rsi < 45 else (0.4 if rsi >= 70 else 0.5)),
    0.6 if 10 < chg24 < 40 else 0.5,
    0.58 if fb in ("mild_short_crowding", "crowded_short") else 0.5,
    0.58 if abs(price - support) / price < 0.015 else 0.48,
    0.65 if mom_1h > 0 else 0.4,
    0.62 if trend_1h_bull else 0.42,
    0.72,
]
...
dd = (high24 - price) / high24 * 100
sup_word = "at" if near_sup else "above"

technical_summary = (
    f"RESOLV +{chg24:.1f}% in 24h on {vol24/1e6:.1f}M USDT volume; price {price:.5f} "
    f"({dd:.1f}% below 24h high {high24:.5f}) {sup_word} support {support:.5f}, "
    f"RSI {rsi:.1f}, EMA9 {ema_dir} EMA21. 1h structure bullish; "
    f"15m pullback with negative funding ({fr*100:.4f}%, shorts pay longs). "
    f"Order book {ob_desc} (imbalance {ob_imb:+.2f}). Strong track record 3W/1L (+1.64 USDT). "
