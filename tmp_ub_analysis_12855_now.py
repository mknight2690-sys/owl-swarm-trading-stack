...
if fr > 0.0005:
    funding_bias = "longs_pay_avoid_long"
elif fr < -0.0005:
    funding_bias = "shorts_pay_avoid_short"
else:
...
if fr > 0.0005:
    ...
    conf -= 0.2
if ta1["bias"] == ta15["bias"] == ta1h["bias"] and ta1["bias"] != "neutral":
    conf += 0.08
else:
    conf -= 0.1
if change24h > 40:
...
supports = sorted(
    set(
        [
            round(ta15["low"], 5),
            round(ta1h["low"], 5),
            round(min(c["low"] for c in c1m[-10:]), 5),
        ]
    )
)[:3]
resists = sorted(
    set(
        [
            round(ta15["high"], 5),
            round(ta1h["high"], 5),
            round(max(c["high"] for c in c1m[-10:]), 5),
        ]
    ),
    reverse=True,
)[:3]
key_levels = supports + resists
...
avoid = spread > 0.5 or fr > 0.0005 or fr < -0.0005
if avoid:
    suggested = "neutral"
    conf = min(conf, 0.49)
elif conf >= 0.5:
    if (
        ta1["bias"] == "long"
        and ta15["bias"] in ("long", "neutral")
        and ta1h["bias"] in ("long", "neutral")
    ):
        suggested = "long"
    elif (
        ta1["bias"] == "short"
        and ta15["bias"] in ("short", "neutral")
        and ta1h["bias"] in ("short", "neutral")
    ):
        suggested = "short"
    else:
        suggested = "neutral"
