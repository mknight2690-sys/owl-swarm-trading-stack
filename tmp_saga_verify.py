...
# Assume prior analysis suggested long (24h +11%, bullish narrative)
claimed_side = "long"
...
if abs(price - 0.01567) / 0.01567 > 0.01:
    issues.append(f"claimed price 0.01567 vs live {price:.5f}")
if abs(change24h - 11.372) > 1.0:
    issues.append(f"claimed 24h change 11.372% vs live {change24h:.2f}%")
if abs(vol - 9757717) / 9757717 > 0.05:
    issues.append(f"claimed volume 9757717 vs live {vol:.0f}")
...
note_bits = [
    ...
    f"live price {price:.5f} 24h {change24h:.2f}%",
    "SAGA-USDT 2W/0L positive track record",
]
if issues:
...
out = {
    "instId": INST,
    "suggestedSide": claimed_side if conf >= 0.5 and spread_ok and favorable_long else "neutral",
    "confidence": conf,
