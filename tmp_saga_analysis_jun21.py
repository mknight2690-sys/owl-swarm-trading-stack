...
support = round(min(ta15["low"], ta1h["low"], ta1["low"]), 5)
resistance = round(max(ta15["high"], ta1h["high"], ta1["high"]), 5)
...
if conf >= 0.5 and spread_ok:
    if aligned_long and fr <= 0.0005:
        suggested = "long"
    elif aligned_short and fr >= -0.0005:
        suggested = "short"
    elif momentum_score >= 0.55 and fr <= 0.0005 and ta1["bias"] != "short" and ta1h["bias"] != "short":
        suggested = "long"
    elif momentum_score <= 0.45 and fr >= -0.0005 and ta1["bias"] != "long" and ta1h["bias"] != "long":
        suggested = "short"
...
out = {
    ...
    "suggestedSide": suggested if suggested != "neutral" else None,
    "confidence": conf,
}
