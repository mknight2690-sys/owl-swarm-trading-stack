...
notes = []
if not checks["funding_favorable_long"]:
    notes.append(f"funding {fr:.6f} expensive for long (longs pay)")
else:
    notes.append(f"funding {fr:.6f} favorable/neutral for long")
if not checks["spread_under_half_pct"]:
    notes.append(f"spread {spread:.4f}% exceeds 0.5% threshold")
else:
    notes.append(f"spread {spread:.4f}% OK")
if not checks["1m_trend_matches_long"]:
    notes.append(f"1m bearish RSI {ta1['rsi']} bias {ta1['bias']} conflicts with long")
else:
    notes.append(f"1m bullish RSI {ta1['rsi']} matches long")
if not checks["15m_momentum_bullish"]:
    notes.append(
        f"15m weak RSI {ta15['rsi']} mom {ta15['mom_pct']:+.2f}% bias {ta15['bias']}"
    )
else:
    notes.append(f"15m momentum OK RSI {ta15['rsi']} mom {ta15['mom_pct']:+.2f}%")
if not checks["1h_trend_bullish"]:
    notes.append(f"1h not bullish RSI {ta1h['rsi']} bias {ta1h['bias']}")
else:
    notes.append(f"1h bullish RSI {ta1h['rsi']} supports long")
notes.append("DRIFT track record 1W/0L +0.3262 PnL but order_blocked=1 recent")
if liq_dist < 5:
    notes.append("HIGH LIQUIDATION RISK")
else:
    notes.append(f"liq distance {liq_dist:.1f}% at 10x isolated")
