...
if abs(price - 0.02271) / 0.02271 > 0.01:
    note_bits.append(f"user price 0.02271 differs from live {price:.5f}")
if abs(change24h - 27.8) > 1:
    note_bits.append(f"user 24h change 27.8% differs from live {change24h:.2f}%")
if abs(vol24h - 55472590) / 55472590 > 0.05:
    note_bits.append(f"user vol 55472590 differs from live {vol24h:.0f}")
if issues:
