...
def bridge(method, args):
    ...
    try:
        ...
        proc = subprocess.run(
            [PY, str(ROOT / "blofin_bridge.py"), method, f"@{path}"],
            cwd=ROOT,
            capture_output=True,
            text=True,
        )
...
ticker_raw = bridge("get_ticker", {"inst_id": inst})
ticker = ticker_raw[0] if isinstance(ticker_raw, list) else ticker_raw

fund_rate = float(fund.get("fundingRate", 0))
...
price = round(float(ticker.get("last", closes(c1m)[-1])), 5)
...
avoid_long = fund_rate > 0.0001
avoid_short = fund_rate < -0.0001
avoid_spread = spread > 0.5

cl1m = closes(c1m)
recent_move = abs(cl1m[-1] - cl1m[-10]) / cl1m[-10] * 100 if len(cl1m) >= 10 else 0
...
if avoid_spread:
    ...
    suggested = "long"
    confidence = 0.58
    if t1 == "bullish" and t15 == "bullish" and t1h == "bullish":
        confidence += 0.15
    elif t1 == "bullish" and t15 == "bullish":
        confidence += 0.10
    if r1 and 45 <= r1 <= 68:
        confidence += 0.05
    if r1 and r1 > 72:
    ...
    suggested = "short"
    confidence = 0.58
    if t1 == "bearish" and t15 == "bearish" and t1h == "bearish":
        confidence += 0.15
...
if avoid_long and suggested == "long":
    confidence = min(confidence, 0.35)
if avoid_short and suggested == "short":
    confidence = min(confidence, 0.35)

confidence = round(min(max(confidence, 0.0), 0.85), 2)
change24h = round(float(ticker.get("chg_pct", 12.027)), 3)
volume24h = float(ticker.get("vol24h", 28538745))

notes = []
if avoid_spread:
    notes.append("wide spread avoid")
if avoid_long:
    notes.append("funding expensive for longs")
if avoid_short:
    notes.append("funding expensive for shorts")

summary = (
    f"1m {t1} RSI {int(r1) if r1 else 'NA'}, 15m {t15} RSI {int(r15) if r15 else 'NA'}, "
    f"1h {t1h} RSI {int(r1h) if r1h else 'NA'}; 24h +{change24h:.1f}%; "
    f"spread {spread:.3f}%; funding {fund_rate:.6f}; MTF {overall}"
)
if notes:
    summary += "; " + ", ".join(notes)
