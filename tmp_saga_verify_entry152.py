...
if abs(price - 0.0152) / 0.0152 > 0.01:
    issues.append(f"claimed price 0.0152 vs live {price:.5f}")
if abs(change24h - 9.827) > 1.0:
    issues.append(f"claimed 24h change 9.827% vs live {change24h:.2f}%")
if abs(vol - 9834448) / 9834448 > 0.05:
    issues.append(f"claimed volume 9834448 vs live {vol:.0f}")
