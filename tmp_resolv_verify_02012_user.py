...
note_bits = [
    ...
    "RESOLV-USDT 6W/4L pnl=1.6295 positive track record; recent close -0.118",
]
if abs(price - 0.02012) / 0.02012 > 0.01:
    note_bits.append(f"price mismatch vs user data ({price:.5f} vs 0.02012)")
if abs(change24h - 15.965) > 1:
    note_bits.append(f"24h change mismatch ({change24h:.2f}% vs 15.965%)")
if issues:
