...
vol24h = float(ticker.get("vol24h", ticker.get("volCcy24h", 11707900)))
...
if change24h < -5:
    conf += 0.05
conf -= 0.15
if not all(x["bias"] == "short" for x in [t1, t15, t1h]):
    conf -= 0.10
...
tech = (
    ...
    f"No MTF alignment; 1h bearish but 1m/15m bullish. Spread {spread:.3f}% OK. "
    f"Funding {fr:.6f} favors shorts. Poor FIGHT track record; skip trade."
)
