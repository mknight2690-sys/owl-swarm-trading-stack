...
ob = bridge("get_order_book", {"inst_id": "UB-USDT"})
fund = bridge("get_funding_rate", {"inst_id": "UB-USDT"})
c1m = bridge("get_candles", {"inst_id": "UB-USDT", "bar": "1m", "limit": 50})
c15 = bridge("get_candles", {"inst_id": "UB-USDT", "bar": "15m", "limit": 30})
c1h = bridge("get_candles", {"inst_id": "UB-USDT", "bar": "1h", "limit": 24})
...
# Extra penalty for extreme 24h pump without clear continuation
if 60.818 > 50 and r1 and r1 > 70:
    confidence -= 0.1

confidence = round(min(max(confidence, 0.0), 0.85), 2)

r1s = f"{r1:.0f}" if r1 is not None else "NA"
r15s = f"{r15:.0f}" if r15 is not None else "NA"
r1hs = f"{r1h:.0f}" if r1h is not None else "NA"
summary = (
    f"1m {t1} RSI {r1s}, 15m {t15} RSI {r15s}, 1h {t1h} RSI {r1hs}; "
    f"24h +60.8% pump; spread {spread:.2f}%; funding {fund_rate:.6f}"
)
