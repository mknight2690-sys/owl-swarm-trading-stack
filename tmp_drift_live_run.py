...
# Positive track record boost for DRIFT
if PROVIDED_LONG > 0.7:
    long_score += 0.05
...
if aligned_bull:
    ...
    factors.append(0.55 if label_1m == "bullish" or label_15 == "bullish" else 0.45)

factors.append(0.65 if 55 < rsi_15 < 70 else (0.35 if rsi_15 >= 70 else (0.6 if rsi_15 < 30 else 0.5)))
factors.append(0.7 if mom_15 > 0 and mom_1h > 0 else (0.35 if mom_15 < 0 and mom_1h < 0 else (0.55 if mom_1m > 0 else 0.45)))
factors.append(0.55 if funding_bias in ("neutral", "mild_short_crowding", "crowded_short") else (0.5 if funding_bias == "mild_long_crowding" else (0.35 if avoid_long else 0.5)))
factors.append(0.2 if avoid_trade else 0.85)
factors.append(0.7 if not avoid_long and long_score >= 0.55 else (0.35 if long_score <= 0.45 else 0.5))
factors.append(min(0.85, PROVIDED_LONG * 0.85))
...
mtf = "aligned bullish" if aligned_bull else ("aligned bearish" if aligned_bear else "mixed MTF")
technical_summary = (
    ...
    f"24h +{chg24:.1f}% from {low24:.5f} low; {mtf}. "
    f"Funding {funding_bias} ({fr*100:.4f}%/8h), spread {spread_pct:.3f}%. "
    f"Support {support:.5f}, resistance {resistance:.5f}. Prior win on DRIFT; long bias {PROVIDED_LONG:.2f}."
)
