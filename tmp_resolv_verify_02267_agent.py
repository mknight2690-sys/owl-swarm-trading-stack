...
note_bits = [
    ...
    "RESOLV-USDT 6W/3L pnl=1.7474 positive track record; recent close -0.358",
]
if abs(price - 0.02267) / 0.02267 > 0.01:
    note_bits.append(f"user price 0.02267 differs from live {price:.5f}")
if abs(change24h - 28.007) > 1:
    note_bits.append(f"user 24h change 28.007% differs from live {change24h:.2f}%")
if abs(vol24h - 55587570) / 55587570 > 0.05:
    note_bits.append(f"user vol 55587570 differs from live {vol24h:.0f}")
if issues:
