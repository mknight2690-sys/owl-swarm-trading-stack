...
key_levels = sorted(
    {
        round(c1h[-1]["low"], 6),
        round(c1h[-1]["high"], 6),
        round(min(c["low"] for c in c1h), 6),
        round(max(c["high"] for c in c1h), 6),
        round(ta15["low"], 6),
        round(ta15["high"], 6),
    }
)
...
if conf >= 0.5 and spread_ok:
    if longs >= 2 and favorable_long and ta1["bias"] != "short":
        suggested = "long"
    elif shorts >= 2 and favorable_short and ta1["bias"] != "long":
        suggested = "short"
    elif trend == "bullish" and favorable_long:
        suggested = "long"
    elif trend == "bearish" and favorable_short:
        suggested = "short"
...
out = {
    ...
    "suggestedSide": suggested if suggested else "neutral",
    "confidence": conf,
}
