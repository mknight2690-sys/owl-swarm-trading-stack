...
if aligned_bull:
    ...
    trend = "bearish"
elif ta1h["suggestedBias"] == "short" and ta1m["suggestedBias"] == "long":
    trend = "bearish_bounce"
...
align_note = "Multi-TF bearish aligned" if aligned_bear else ("Multi-TF bullish aligned" if aligned_bull else "Timeframes mixed")
summary = (
    f"1m RSI {ta1m['rsi14']} {ta1m['suggestedBias']}; "
    f"15m RSI {ta15['rsi14']} {ta15['suggestedBias']}; "
    f"1h RSI {ta1h['rsi14']} {ta1h['suggestedBias']}. "
    f"24h {change24h:.2f}%; funding {fr:.6f}; spread {spread:.3f}%. "
    f"{align_note}. Spread >0.5% blocks trade; funding <-0.05% penalizes shorts."
)
...
if conf >= 0.5 and not avoid_trade:
    if aligned_bear and not avoid_short:
        out["suggestedSide"] = "short"
    elif aligned_bull and not avoid_long:
        out["suggestedSide"] = "long"
    elif ta1h["suggestedBias"] == "short" and ta15["suggestedBias"] == "short" and not avoid_short:
        out["suggestedSide"] = "short"
    elif ta1h["suggestedBias"] == "long" and ta15["suggestedBias"] == "long" and not avoid_long:
        out["suggestedSide"] = "long"
