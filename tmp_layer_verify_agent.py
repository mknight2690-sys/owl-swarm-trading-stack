...
ENTRY = 0.0841
LEV = 10
CLAIMED = {"price": 0.0841, "change24h": 26.848, "volume": 6010007, "side": "long"}
...
vol = float(ticker.get("volCurrency24h", ticker.get("volCcy24h", 0)))
...
if fr > 0.0005:
    conf -= 0.2
aligned_long = (
    ta1["bias"] == "long" and ta15["bias"] == "long" and ta1h["bias"] == "long"
)
aligned_short = (
    ta1["bias"] == "short" and ta15["bias"] == "short" and ta1h["bias"] == "short"
)
if aligned_long or aligned_short:
    conf += 0.08
else:
    conf -= 0.1
if ta15["rsi"] and ta15["rsi"] > 75:
    conf -= 0.08
...
if claimed_side == "long" and not favorable_long:
    issues.append(f"funding rate {fr:.6f} unfavorable for long")
...
if ta15["rsi"] and ta15["rsi"] > 75:
    issues.append(f"15m RSI {ta15['rsi']} overbought")
if ta1h["rsi"] and ta1h["rsi"] > 75:
    issues.append(f"1h RSI {ta1h['rsi']} overbought")
if liq_dist < 5:
    issues.append("HIGH LIQUIDATION RISK")
if abs(price - CLAIMED["price"]) / CLAIMED["price"] > 0.01:
    issues.append(f"claimed price {CLAIMED['price']} vs live {price:.5f}")
if abs(change24h - CLAIMED["change24h"]) > 1.0:
    issues.append(
        f"claimed 24h change {CLAIMED['change24h']}% vs live {change24h:.2f}%"
    )

verified = len(issues) == 0
suggested = (
    claimed_side
    if conf >= 0.5 and spread_ok and favorable_long and ta1["bias"] != "short"
    else "neutral"
)

note_bits = [
    f"1m {ta1['bias']} RSI {ta1['rsi']} chg {ta1['chg']:+.2f}%",
    f"15m {ta15['bias']} RSI {ta15['rsi']} chg {ta15['chg']:+.2f}%",
    f"1h {ta1h['bias']} RSI {ta1h['rsi']} chg {ta1h['chg']:+.2f}%",
    f"funding {fr:.6f} favorable for long",
    f"live price {price:.5f} 24h {change24h:.2f}% vol {vol:.0f}",
    "no LAYER-USDT track record",
]
if issues:
