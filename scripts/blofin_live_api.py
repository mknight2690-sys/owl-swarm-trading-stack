...
def fetch_live_account(*, force: bool = False, min_interval_sec: float = 2.0) -> dict[str, Any]:
    ...
    CRITICAL: When module cache is empty (fresh start), reads disk cache FIRST
    before calling the API. This prevents rate-limit hammering on restart.
    """
    global _last_fetch_ts, _last_positions, _last_equity, _last_available, _last_fetch_ok
    now = time.time()

    # ── CRITICAL FIX: Fresh start with empty module cache — load from disk first ──
    if _last_positions is None:
        try:
            if POSITIONS_CACHE.is_file():
                raw = json.loads(POSITIONS_CACHE.read_text(encoding="utf-8"))
                rows = list(raw.get("open_rows") or [])
                if rows:
                    _last_positions = rows
                    _last_fetch_ts = float(raw.get("updated_at") or 0)
                    _last_fetch_ok = True
                    # Also load equity from disk
                    if EQUITY_CACHE.is_file():
                        eq_raw = json.loads(EQUITY_CACHE.read_text(encoding="utf-8"))
                        _last_equity = float(eq_raw.get("equity_usdt") or 0)
                        _last_available = float(eq_raw.get("available_usdt") or 0)
        except Exception:
            pass
