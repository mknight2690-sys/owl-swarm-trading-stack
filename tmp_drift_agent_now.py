...
support = min(c["low"] for c in c1h[-6:])
resistance = max(c["high"] for c in c1h[-6:])
...
if conf >= 0.5 and spread_ok and favorable_long and aligned_long:
    ...
    suggested = "short"
elif conf >= 0.5 and spread_ok and long_count >= 2 and favorable_long:
    suggested = "long"
elif conf >= 0.5 and spread_ok and short_count >= 2 and favorable_short:
    suggested = "short"
else:
    suggested = None
