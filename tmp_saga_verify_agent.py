...
if abs(price - 0.0153) / 0.0153 > 0.01:
    issues.append(f"claimed price 0.0153 vs live {price:.5f}")
if abs(change24h - 9.835) > 1.0:
    issues.append(f"claimed 24h change 9.835% vs live {change24h:.2f}%")
if abs(vol - 9762383) / 9762383 > 0.05:
    issues.append(f"claimed volume 9762383 vs live {vol:.0f}")
