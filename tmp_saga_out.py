...
supports = sorted(set([round(ta1["low"], 5), round(ta15["low"], 5), round(ta1h["low"], 5), round(min(c["low"] for c in c1h), 5)]))
resists = sorted(set([round(ta1["high"], 5), round(ta15["high"], 5), round(ta1h["high"], 5), round(max(c["high"] for c in c1h), 5)]))
key_levels = supports[:2] + [round(price, 5)] + resists[-2:]

momentum = round(ta1["tech"] * 0.4 + ta15["tech"] * 0.35 + ta1h["tech"] * 0.25, 3)

align_long = ta1["bias"] == "long" and ta15["bias"] == "long" and ta1h["bias"] == "long"
align_short = ta1["bias"] == "short" and ta15["bias"] == "short" and ta1h["bias"] == "short"

if align_long:
    trend = "bullish"
elif align_short:
    trend = "bearish"
elif ta1h["bias"] == "long" or ta15["bias"] == "long":
    trend = "mixed_bullish"
elif ta1h["bias"] == "short" or ta15["bias"] == "short":
    trend = "mixed_bearish"
else:
    trend = "neutral"
...
if spread_ok and conf >= 0.5:
    if momentum >= 0.55 and favorable_long:
        suggested = "long"
    elif momentum <= 0.45 and favorable_short:
        suggested = "short"

mtf = "bearish MTF aligned" if align_short else ("bullish MTF aligned" if align_long else "no MTF alignment")
tech_sum = (
    f"1m {ta1['bias']} RSI {ta1['rsi']}; "
    f"15m {ta15['bias']} RSI {ta15['rsi']} oversold; "
    f"1h {ta1h['bias']} RSI {ta1h['rsi']}; "
    f"24h +{change24h:.1f}%; spread {spread:.3f}% OK; "
    f"funding {fr:.6f} longs pay; {mtf}; pullback within +10% rally"
)
