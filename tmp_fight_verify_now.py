...
support = round(min(ta1["low"], ta15["low"], ta1h["low"]), 6)
resist = round(max(ta1["high"], ta15["high"], ta1h["high"]), 6)
key_levels = sorted(set([support, round(price * 0.99, 6), round(price, 6), round(price * 1.01, 6), resist]))
...
if aligned_long:
    ...
    conf -= 0.1
# FIGHT-USDT poor track record penalty
conf -= 0.15
conf = max(0, min(1, round(conf, 3)))

suggested = None
if conf >= 0.5 and spread_ok and favorable_short and (aligned_short or short_c >= 2):
    suggested = "short"
elif conf >= 0.5 and spread_ok and favorable_long and (aligned_long or long_c >= 2):
    suggested = "long"
