...
if abs(change24h - CLAIMED_24H) > 2.0:
    issues.append(f"claimed 24h change {CLAIMED_24H}% vs live {change24h:.2f}%")
if CLAIMED_VOL > 0 and abs(vol24h - CLAIMED_VOL) / CLAIMED_VOL > 0.05:
    issues.append(f"claimed volume {CLAIMED_VOL} vs live {vol24h:.0f}")
...
note_bits = [
    ...
    "RESOLV-USDT 4W/2L pnl=1.6891 positive track record",
]
if issues:
