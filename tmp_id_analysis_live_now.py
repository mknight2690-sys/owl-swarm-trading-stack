...
if fr > 0.0005:
    funding_bias = "long_avoid"
elif fr < -0.0005:
    funding_bias = "short_avoid"
else:
...
if aligned_long and spread_ok and favorable_long:
    suggested = "long"
elif aligned_short and spread_ok and favorable_short:
    suggested = "short"
else:
    avg = (ta1["tech"] + ta15["tech"] + ta1h["tech"]) / 3
    if avg >= 0.55 and spread_ok and favorable_long:
        suggested = "long"
    elif avg <= 0.45 and spread_ok and favorable_short:
        suggested = "short"
    else:
        suggested = "long" if avg >= 0.5 else "short"

if conf < 0.5:
    suggested = "long" if suggested == "long" else "short"

overall = "bullish" if aligned_long else ("bearish" if aligned_short else "mixed")
...
tech_sum = (
    ...
    f"spread {spread:.4f}%; funding {fr:.6f}; 24h +{change24h:.1f}%"
)
