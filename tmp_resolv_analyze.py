...
trend_label = "bullish" if avg_trend >= 0.6 else ("bearish" if avg_trend <= 0.4 else "neutral")
...
factors = [
    0.75 if bull_count >= 2 else (0.35 if bear_count >= 2 else 0.5),
    0.65 if rsi15 and 45 < rsi15 < 70 else (0.4 if rsi15 and rsi15 >= 70 else 0.5),
    0.7 if not avoid_trade else 0.2,
    0.65
    if not (avoid_long and suggested == "long") and not (avoid_short and suggested == "short")
    else 0.35,
    0.6 if abs(mom_15) > 0.5 or abs(mom_1h) > 0.5 else 0.45,
    0.62 if aligned else 0.42,
]
...
if suggested == "neutral":
    if bull_count >= 2 and not avoid_long and long_score >= 0.5:
        suggested = "long"
        confidence = max(0.52, confidence - 0.03)
    elif bear_count >= 2 and not avoid_short and long_score <= 0.5:
        suggested = "short"
        confidence = max(0.52, confidence - 0.03)
    else:
...
rsi1m_s = f"{rsi1m:.1f}" if rsi1m else "n/a"
rsi15_s = f"{rsi15:.1f}" if rsi15 else "n/a"
technical_summary = (
    f"RESOLV-USDT at {price:.5f} ({chg24:+.2f}% 24h). Spread {spread_pct:.3f}%. "
    f"Funding {fr * 100:.4f}% ({fb}). 1m RSI {rsi1m_s}, 15m RSI {rsi15_s}. "
