...
vol = ticker.get("volCurrency24h", ticker.get("vol24h"))
...
notes_parts = []
if not funding_favorable_short:
    notes_parts.append(f"ERROR: funding {fr:.6f} positive (shorts pay); not favorable for short")
else:
    notes_parts.append(f"funding {fr:.6f} favorable for short")

if not trend_match_short:
    notes_parts.append(
        f"ERROR: timeframe mismatch 1m={t1['bias']} 15m={t15['bias']} 1h={t1h['bias']}; short needs all bearish"
    )
else:
...
notes_parts.append(f"liq {liq_price:.6f} distance {liq_dist:.1f}% at 10x isolated short")
if liq_dist < 5:
    notes_parts.append("HIGH LIQUIDATION RISK")

notes_parts.append(f"live price {price} 24h {change24h:.2f}% vol {vol}")
notes_parts.append("FIGHT-USDT on avoid list; recent closed short pnl=-0.01048")

if abs(price - ENTRY) / ENTRY > 0.01:
    notes_parts.append(f"ERROR: user price {ENTRY} differs from live {price}")
