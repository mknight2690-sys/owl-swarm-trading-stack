...
vol24h = float(ticker.get("vol24h", ticker.get("volCcy24h", 0)))
...
if fr > 0.0005:
    ...
    fb = "slightly_bearish_longs"
elif fr < 0:
    fb = "slightly_bearish_shorts"
else:
    fb = "neutral"
...
if conf >= 0.5 and spread_ok and favorable_long and aligned_long:
    suggested = "long"
elif conf >= 0.5 and spread_ok and favorable_short and aligned_short:
    suggested = "short"
elif conf >= 0.5 and spread_ok and favorable_long and long_count >= 2:
    suggested = "long"
elif conf >= 0.5 and spread_ok and favorable_short and short_count >= 2:
    suggested = "short"
else:
    suggested = "neutral"
...
tech_sum = (
    ...
    f"spread {spread:.4f}% | funding {fr:.6f} | DOOD 2W/0L track record"
)
...
out = {
    ...
    "suggestedSide": suggested if suggested != "neutral" and conf >= 0.5 else suggested,
    "confidence": conf,
}

if out["suggestedSide"] == "neutral" or conf < 0.5:
    out.pop("suggestedSide", None)
