...
def signed_request(method, path, body_obj=None):
    ...
    url = BASE + path
    req = urllib.request.Request(url, data=body.encode() if body else None, headers=h, method=method)
    try:
...
def get_equity():
    try:
        data = signed_request("GET", "/api/v1/account/balance?accountType=futures")
        d = data.get("data")
...
def get_positions():
    try:
        data = signed_request("GET", "/api/v1/account/positions?accountType=futures")
        rows = data.get("data")
        if isinstance(rows, list):
            return [r for r in rows if float(r.get("pos", 0)) != 0]
        elif isinstance(rows, dict):
            return [rows] if float(rows.get("pos", 0)) != 0 else []
    except Exception as e:
...
def main():
    ...
    # Close stale (>5 min) or TP/SL hit
    ...
    for coin in COINS:
        ...
        if len(candles) < 5:
            continue
        # compute momentum: last 5 candles, weighted toward most recent
        closes = [c["close"] for c in candles[-5:]]
        opens = [c["open"] for c in candles[-5:]]
        pct = (closes[-1] - closes[0]) / closes[0] * 100 if closes[0] else 0
        # recent 3-minute streak
        recent = candles[-3:]
    ...
    # Size: 5-10% equity with 10x leverage
