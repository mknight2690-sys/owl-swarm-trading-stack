...
issues.append("ID-USDT prior order_blocked; no positive track record (0 trades, blocked=1)")

note_bits = [
    f"1m {ta1['bias']} RSI {ta1['rsi']} mom {ta1['mom_pct']:+.2f}%",
    f"15m {ta15['bias']} RSI {ta15['rsi']} mom {ta15['mom_pct']:+.2f}%",
    f"1h {ta1h['bias']} RSI {ta1h['rsi']} mom {ta1h['mom_pct']:+.2f}%",
...
if aligned_long and spread_ok and favorable_long:
    suggested = "long"
elif aligned_short and spread_ok:
    suggested = "short"
elif (ta1["tech"] + ta15["tech"] + ta1h["tech"]) / 3 >= 0.55 and favorable_long:
    suggested = "long"
elif (ta1["tech"] + ta15["tech"] + ta1h["tech"]) / 3 <= 0.45:
    suggested = "short"
else:
    suggested = "neutral"

core_issues = [i for i in issues if "order_blocked" not in i]
verified = len(core_issues) == 0
