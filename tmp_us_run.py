...
if ema9 < ema21 and price < ema21: trend = "bearish"
elif ema9 > ema21 and price > ema21: trend = "bullish"
else: trend = "neutral"

momentum = 0.35 if rsi < 45 else (0.65 if rsi > 55 else 0.5)
if fr > 0.0001: fb = "mild_long_crowding"
elif fr < -0.0001: fb = "mild_short_crowding"
else: fb = "neutral"

factors = [PRE_SHORT, 0.70 if ema9 < ema21 else 0.35, 0.70 if chg24 < -5 else 0.45,
    0.65 if price < pivot else 0.45, 0.45 if abs(price - support) / price < 0.008 else 0.55,
    0.50 if rsi < 35 else (0.55 if rsi < 45 else 0.5), 0.60 if mom_4h < 0 else 0.45,
...
technical_summary = (
    ...
    f"resistance {resistance:.5f}, pivot {pivot:.5f}. "
    f"Pre-rank short_score {PRE_SHORT} vs long_score {PRE_LONG}; no adverse trade history."
)

print(json.dumps({
    "instId": INST, "price": round(price, 5), "change24h": round(chg24, 3), "volume24h": vol24,
    "trend": trend, "momentumScore": round(momentum, 3),
    "keyLevels": [
        {"type": "support", "price": round(support, 5)},
        {"type": "resistance", "price": round(resistance, 5)},
        {"type": "pivot", "price": round(pivot, 5)},
        {"type": "24h_low", "price": round(low24, 5)},
        {"type": "24h_high", "price": round(high24, 5)},
    ],
    "fundingRate": round(fr, 6), "fundingBias": fb, "technicalSummary": technical_summary,
    "suggestedSide": suggested, "confidence": confidence,
}))
