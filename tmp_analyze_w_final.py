...
price = round(float(ticker.get("last", cl1m[-1])), 5)
...
change24h = round(float(ticker.get("chg_pct", 12.183)), 3)
volume24h = float(ticker.get("vol24h", 28804748))

summary = (
    f"1m {t1} RSI {fmt_r(r1)}, 15m {t15} RSI {fmt_r(r15)}, 1h {t1h} RSI {fmt_r(r1h)}; "
    f"24h +{change24h:.1f}%; spread {spread:.3f}%; funding benign; MTF aligned bullish"
)
