...
factors = [
    0.65 if ema9 > ema21 else 0.35,
    0.65 if rsi > 55 and rsi < 70 else (0.4 if rsi >= 70 else 0.5),
    0.6 if chg24 > 5 else 0.45,
    0.55 if fb in ("mild_short_crowding", "crowded_short") else (0.45 if fb == "mild_long_crowding" else 0.5),
    0.6 if ob_imb > 0.05 else (0.4 if ob_imb < -0.05 else 0.5),
...
if fr < 0:
    fund_note = f"Funding negative ({fr * 100:.4f}%) favors longs receiving payments"
elif fr > 0:
    fund_note = f"Funding positive ({fr * 100:.4f}%) favors shorts receiving payments"
else:
    fund_note = "Funding near neutral"

technical_summary = (
    f"{INST} {chg24:.1f}% in 24h from {low24:.6f} low to {high24:.6f} high; "
    f"price {price:.6f} {sup_word} session support {support:.6f} "
    f"with RSI {rsi:.1f}, EMA9 {ema_dir} EMA21. "
    f"{fund_note}; order book {ob_desc} (imbalance {ob_imb:+.2f}). "
    f"1h momentum {mom_1h:+.2f}%, 4h {mom_4h:+.2f}%; resistance near {resistance:.6f}, pivot {pivot:.6f}."
)
