...
track_record_bad = True  # W/L=0/1 pnl=-0.0105 per user history
...
if not trend_match_short:
    notes_parts.append(
        f"ERROR: trend mismatch for short (1m={t1['bias']}, 15m={t15['bias']}, 1h={t1h['bias']})"
    )
else:
    notes_parts.append("all timeframes bearish/short aligned")
if spread_ok:
    notes_parts.append(f"spread {spread:.4f}% OK (below 0.5%)")
else:
    notes_parts.append(f"ERROR: spread {spread:.4f}% exceeds 0.5%")
notes_parts.append(f"liq distance {liq_dist:.2f}% at 10x isolated short entry {ENTRY}")
if liq_dist < 5:
    notes_parts.append("HIGH LIQUIDATION RISK")
notes_parts.append(f"live price {price} 24h {change24h:.2f}%")
notes_parts.append(
    "HALLUCINATION/ASSUMPTION: prior agent omitted FIGHT-USDT poor track record (1 trade W/L=0/1 pnl=-0.0105); avoid repeat loser"
)
