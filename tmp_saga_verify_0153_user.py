...
ENTRY = 0.0153
LEV = 10
CLAIMED = {"price": 0.0153, "change24h": 9.677, "volume": 11257927, "side": "long"}
...
if claimed_side == "long" and not favorable_long:
    issues.append(f"funding rate {fr:.6f} > 0.0005 unfavorable for long")
...
if abs(vol - CLAIMED["volume"]) / CLAIMED["volume"] > 0.05:
    issues.append(f"claimed volume {CLAIMED['volume']} vs live {vol:.0f}")

verified = len(issues) == 0
suggested = (
    claimed_side if conf >= 0.5 and spread_ok and favorable_long else "neutral"
)
...
note_bits = [
    ...
    "SAGA-USDT 3W/0L positive track record",
]
if issues:
