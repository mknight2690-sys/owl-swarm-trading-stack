...
short_score = 1 - long_score

factors = [
    0.35 if ema9 < ema21 else 0.65,
    0.65 if rsi < 45 else (0.4 if rsi >= 70 else 0.5),
    0.65 if chg24 < -10 else 0.5,
    0.55 if fb in ("mild_long_crowding", "crowded_long") else (0.5 if fb == "neutral" else 0.45),
    0.6 if abs(price - support) / price < 0.015 else 0.55,
...
suggested = "short" if short_score >= 0.55 else ("long" if long_score >= 0.55 else "short")
...
technical_summary = (
    ...
    f"price {price:.5f} {'near' if near_sup else 'above'} session support {support:.5f} "
...
report = {
    ...
    "volume24h": int(vol24),
    "trend": trend_label,
    "momentumScore": round(1 - momentum if chg24 < 0 else momentum, 3),
